...
public final class PackageProcessor {
    ...
    public init(inputPath: String, outputPath: String) throws {
        ...
        let path = URL(fileURLWithPath: inputPath).appendingPathComponent("Package.swift").path
        ...
        self.packageDump = try PackageLoader.loadPackageDump(from: inputPath)
        self.packageDependencyTree = try PackageLoader.loadPackageDependencies(from: inputPath)
    }
    ...
    public func run() throws {
        ...
        if canRenderDOT {
            try ModuleGraphGenerator(basePath: outputPath, packageDump: packageDump).run()
            try DependencyGraphGenerator(basePath: outputPath, dependencyTree: packageDependencyTree).run()
        }
        ...
        let content: [MarkdownConvertible] = [
            ...
            "This file was generated by [SourceDocs](https://github.com/eneko/SourceDocs) on \(Date().description)"
        ]

        let file = MarkdownFile(filename: "Package", basePath: outputPath, content: content)
    ...
    func createOutputFolderIfNeeded() throws {
        ...
        if FileManager.default.fileExists(atPath: outputPath, isDirectory: &isDir) == false {
